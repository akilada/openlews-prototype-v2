...
class NSDIDownloader:
    ...
    def download_all_features(
        self, batch_size: int = 1000, bounds: Optional[tuple] = None
    ) -> List[Dict]:
        """
        ...
        if bounds:
            ...
            print(
                f"  🗺️  Applying bounds filter: lat [{min_lat}, {max_lat}], lon [{min_lon}, {max_lon}]"
            )
        ...
        while True:
            ...
            try:
                ...
                if not features:
                    print(
                        f"  ✅ Download complete: {len(all_features):,} total records"
                    )
                    break
                ...
                if offset % 10000 == 0:
                    print(
                        f"  📊 Progress: {len(all_features):,} features downloaded..."
                    )
...
class DynamoDBIngester:
    ...
    def ingest_items(
        self, items: List[Dict], batch_size: int = 25, dry_run: bool = False
    ) -> int:
        ...
        if dry_run:
            ...
            oversized = [
                (item["zone_id"], estimate_item_size(item))
                for item in items
                if estimate_item_size(item) > 400 * 1024
            ]
            if oversized:
        ...
        for i in range(0, len(items), batch_size):
            ...
            if batch_num % 100 == 0 or batch_num == total_batches:
                print(
                    f"  ✓ Batch {batch_num:,}/{total_batches:,} ({inserted_count:,}/{len(items):,})"
                )

            time.sleep(0.05)

        print(
            f"✅ DynamoDB ingestion complete: {inserted_count:,}/{len(items):,} items"
        )
...
class PineconeIngester:
    ...
    def __init__(self, api_key: Optional[str], index_name: str):
        ...
        if api_key:
            try:
                ...
                print(
                    "  ⚠️  pinecone-client not installed. Run: pip install pinecone-client"
                )
            except Exception as e:
    ...
    def ingest_vectors(
        ...
        for i in range(0, len(vectors), batch_size):
            ...
            try:
                ...
                if batch_num % 50 == 0 or batch_num == total_batches:
                    print(
                        f"  ✓ Batch {batch_num:,}/{total_batches:,} ({upserted_count:,}/{len(vectors):,})"
                    )
            except Exception as e:
        ...
        print(
            f"✅ Pinecone ingestion complete: {upserted_count:,}/{len(vectors):,} vectors"
        )
...
def main():
    ...
    parser.add_argument(
        "--skip-pinecone", action="store_true", help="Skip Pinecone upload"
    )
    parser.add_argument(
        "--skip-dynamodb", action="store_true", help="Skip DynamoDB upload"
    )
    parser.add_argument(
        "--skip-backup", action="store_true", help="Skip saving backup file"
    )
    parser.add_argument(
    ...
    print(
        f"  Geohash Library: {'pygeohash' if GeoHashCalculator.is_real_geohash() else 'simple fallback'}"
    )
    if args.dry_run:
        print("  ⚠️  DRY RUN MODE - No data will be written")
    ...
    if args.embedding_method != "none" and not args.skip_pinecone:
        ...
        embedding_gen = EmbeddingGenerator(
            method=args.embedding_method, model=EMBEDDING_MODEL
        )
    ...
    if not args.skip_pinecone and PINECONE_API_KEY and embeddings:
        ...
        pinecone_ingester.ingest_vectors(
            processed_items, embeddings, dry_run=args.dry_run
        )
    ...
    print(
        f"     aws dynamodb scan --table-name {DYNAMODB_TABLE_NAME} --max-items 5 --region {AWS_REGION}"
    )
