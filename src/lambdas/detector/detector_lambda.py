...
BEDROCK_MODEL_ID = os.environ.get('BEDROCK_MODEL_ID', 'apac.anthropic.claude-3-haiku-20240307-v1:0')
...
def analyze_sensors(telemetry_data: Dict[str, List[Dict]]) -> Dict:
    ...
    for sensor_id in sensor_risks:
        ...
        composite_risk = fusion_algorithm.calculate_composite_risk(
            sensor_risks[sensor_id]['risk_score'],
            correlation
        )
...
async def process_cluster(cluster: Dict, analysis: Dict) -> Optional[Dict]:
    ...
    logger.info(f"Processing high-risk cluster: {cluster_id}", extra={
...
async def process_individual_sensor(
    ...
    Returns:
        Alert data if created/escalated, None otherwise
    """
    logger.info(f"Processing high-risk sensor: {sensor_id}", extra={
        'composite_risk': sensor_data['composite_risk']
    })
    ...
    if existing_alert:
        ...
        if llm_output['risk_level'] in ['Orange', 'Red']:
            narrative = await bedrock_client.generate_narrative(
                llm_output,
                {'sensor_id': sensor_id, 'telemetry': telemetry, "location": resolved_location, "latitude": lat, "longitude": lon,},
                rag_context
            )
        
        resolved_location = location_resolver.resolve(float(telemetry["latitude"]), float(telemetry["longitude"]))
...
def lambda_handler(event: Dict, context: LambdaContext) -> Dict:
    ...
    Returns:
        Execution summary
    """
    logger.info("Starting detector analysis", extra={
        'event': event,
    ...
    try:
        ...
        logger.info("Analysis complete", extra={
            ...
            'high_risk_count': sum(
                1 for s in analysis['sensor_risks'].values() 
        ...
        return {
            ...
            'body': json.dumps({
                ...
                'sensors_analyzed': len(analysis['sensor_risks']),
                'clusters_detected': len(analysis['clusters']),
                'alerts_created': sum(1 for a in alerts if a.get('action') == 'created'),
                'alerts_escalated': sum(1 for a in alerts if a.get('action') == 'escalated'),
