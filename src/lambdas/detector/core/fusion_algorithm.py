...
class FusionAlgorithm:
    ...
    def calculate_spatial_correlation(
        ...
        neighbours = self._find_neighbours(
            sensor_lat, sensor_lon, sensor_id, telemetry_data, self.CORRELATION_RADIUS_M
        )
        ...
        logger.debug(
            f"Sensor {sensor_id} correlation",
            extra={
    ...
    def calculate_composite_risk(
        self, individual_risk: float, spatial_correlation: float
    ) -> float:
        """
        ...
        if spatial_correlation > 0.6:
            ...
            logger.debug(
                "High correlation - boosting risk",
                extra={"individual": individual_risk, "composite": composite},
            )
        elif spatial_correlation < 0.3:
            # Isolated anomaly = likely sensor fault
            composite = individual_risk * 0.5
            logger.debug(
                "Low correlation - reducing risk (possible fault)",
                extra={"individual": individual_risk, "composite": composite},
            )
        else:
            # Moderate agreement
    ...
    def detect_clusters(
        self, sensor_risks: Dict[str, Dict], telemetry_data: Dict[str, List[Dict]]
    ) -> List[Dict]:
        """
        ...
        for sensor_id, data in sorted_sensors:
            ...
            if len(high_risk_neighbours) >= 2:
                ...
                cluster = {
                    ...
                    "center_location": {"lat": sensor_lat, "lon": sensor_lon},
                ...
                logger.info(
                    f"Cluster detected: {sensor_id}",
                    extra={"size": len(cluster_members), "avg_risk": avg_risk},
                )
    ...
    def _find_high_risk_neighbours(
        ...
        high_risk_neighbours = [
            ...
            if sid in sensor_risks
            and sensor_risks[sid].get("composite_risk", 0) >= risk_threshold
        ]
