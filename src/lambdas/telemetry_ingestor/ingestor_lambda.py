    ...
    def validate(cls, telemetry: Dict) -> Tuple[bool, Optional[str]]:
        ...
        if isinstance(timestamp, str):
            try:
                ...
                return False, f"timestamp string must be ISO 8601 or Unix epoch number, got {timestamp}"
        ...
        if not isinstance(telemetry['sensor_id'], str) or len(telemetry['sensor_id']) < 3:
...
def point_in_bbox(lat: float, lon: float, bbox: Dict) -> bool:
    try:
        return (
            bbox["min_lat"] <= Decimal(str(lat)) <= bbox["max_lat"] and
            bbox["min_lon"] <= Decimal(str(lon)) <= bbox["max_lon"]
        )
    except Exception:
        return False
...
class NDISEnricher:
    ...
    def _pick_best_zone(self, zones: list, lat: float, lon: float) -> dict | None:
        contained = [z for z in zones if self._bbox_contains(z.get("bounding_box"), lat, lon)]
    ...
    def get_hazard_zone(self, geohash: str, latitude: float, longitude: float) -> Optional[Dict]:
        ...
        if zones is None:
            try:
                ...
                logger.error(f"Error querying hazard zones (index={self.index_name} key={self.index_key}): {e}")
                return None
...
class TelemetryWriter:
    ...
    def write_batch(self, telemetry_batch: List[Dict]) -> Dict:
        stats = {'total': len(telemetry_batch), 'succeeded': 0, 'failed': 0, 'errors': []}
        ...
        with self.table.batch_writer() as writer:
            for item in batch:
                try:
                    ...
                    stats['failed'] += 1
                    stats['errors'].append({'sensor_id': item.get('sensor_id'), 'error': str(e)})
...
class EventBridgePublisher:
    ...
    def is_high_risk(cls, telemetry: Dict) -> bool:
        if telemetry.get('moisture_percent', 0) >= cls.HIGH_RISK_THRESHOLDS['moisture_percent']:
            return True
        if telemetry.get('pore_pressure_kpa', 0) >= cls.HIGH_RISK_THRESHOLDS['pore_pressure_kpa']:
            return True
        if telemetry.get('tilt_rate_mm_hr', 0) >= cls.HIGH_RISK_THRESHOLDS['tilt_rate_mm_hr']:
            return True
        if 0 < telemetry.get('safety_factor', 10) < cls.HIGH_RISK_THRESHOLDS['safety_factor']:
            return True

        ndis = telemetry.get('ndis_enrichment', {})
        if ndis.get('hazard_level') in ['High', 'Very High'] and telemetry.get('moisture_percent', 0) > 70:
            return True
        ...
        try:
            event = {
                ...
                'Detail': json.dumps({
                    ...
                    'hazard_level': telemetry.get('ndis_enrichment', {}).get('hazard_level'),
